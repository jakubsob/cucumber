...
**Note:** Tags placed above `Background` or steps (`Given`, `When`, `Then`, `And`, `But`, `*`) will cause a validation error. Place tags only on the elements listed above.
...
Tags placed in invalid locations will result in an error. For example, this is **invalid** and will throw an error:
...
**Note:** It is not possible to place tags above `Background` or steps (`Given`, `When`, `Then`, `And` and `But`).

## Tag inheritance

Tags are inherited by child elements. Tags placed above a `Feature` will be inherited by `Scenario`, `Scenario Outline`, or `Examples`. Similarly, tags placed above a `Scenario Outline` will be inherited by `Examples`.

## Running a subset of scenarios

You can use the `tags` parameter in `cucumber::test()` or `cucumber::run()` to only run scenarios with particular tags.
...
For more advanced tag expressions you can use parentheses for clarity, or to change operator precedence:
...
**Operator precedence:** `not` has the highest precedence, followed by `and`, then `or`. Use parentheses to change the order of evaluation.

The operators are case-insensitive (`and`, `AND`, and `And` all work the same way).
...
You can use custom reporting to turn these tags into links pointing to documents in your external tool.
