...
#' @import stringr
#' @import purrr
extract_params <- function(input, template, parameters = getOption("parameters")) {
  match_template <- replace_docstring(template, parameters)
  find_types_pattern <- paste0(names(parameters$mapper), collapse = "|")
...
#' @import stringr
find_definition <- function(input, template, parameters = getOption("parameters")) {
...
replace_docstring <- function(input, parameters = getOption("parameters")) {
  mapper <- unlist(parameters$mapper)
  str_replace_all(input, mapper)
}
