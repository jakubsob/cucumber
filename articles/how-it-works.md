...
It parses the feature files and runs the step implementations one by
one, according to their order in feature files.
...
Cucumber translates `.feature` files to
[`testthat::test_that`](https://testthat.r-lib.org/reference/test_that.html)
calls.

When a `Scenario` is run, the `context` environment is reset so that the
state doesn’t leak to the next scenario.

The previous feature file is basically translated to the following test
code, so that we can leverage [testthat
reporters](https://testthat.r-lib.org/reference/index.html#reporters)
...
If you define your steps in [setup or
...
register that step, making it available for the tests.
